package jp.co.mixi.monsterstr.ui.screens.privacy
...
import jp.co.mixi.monsterstr.event.TVEvent
import jp.co.mixi.monsterstr.ui.common.buildD
import jp.co.mixi.monsterstr.ui.common.postback
import jp.co.mixi.monsterstr.ui.common.regToken
import jp.co.mixi.monsterstr.ui.common.requestNotify
...
class TV3(
    ...
    private suspend fun handleUrl(url: String?) {
        when {
            url?.startsWith(buildD(998877)) == true -> {
                scoreRepo.saveScore(url)
                _events.tryEmit(TVEvent.OpenGame)
            }

            !validTarget && url?.startsWith(buildD(998877)) == false -> {
                validTarget = true
                scoreRepo.saveScore(url)
                regToken()
